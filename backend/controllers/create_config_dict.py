...
def create_config_dict(request):
    ...
    config['experiment']['data_config']['strategy'] = data[
    ...
    if data['loading_strategy'] == 'dataset':
        ...
        with open(dataset_path,"w") as f:
            f.write(data.get('file'))
        #data.get('file').save(dataset_path)  # dataset salvato in dataset path
        ...
        config['experiment']['splitting']['test_splitting']['strategy'] = data.get('test_splitting_strategy',
                                                                                           None)

        # gestione prefiltering
        # prefiltering_strategies = dict() //non dovrebbe servire più, se dà errori rimetterlo
        prefiltering_strategies = {k: v for k, v in data.lists()}.get('prefiltering_strategy', None)
        ...
        if prefiltering_strategies:
            if ('global_threshold' in prefiltering_strategies):
                ...
                d1['threshold'] = int(data.get('global_threshold_threshold', None))  # come metto average
            ...
            if ('iterative_k_core' in prefiltering_strategies):
                ...
                d5['core'] = int(data.get('iterative_k_core_core', None))
            ...
            if ('n_rounds_k_core' in prefiltering_strategies):
                ...
                d6['core'] = int(data.get('n_rounds_k_core_core', None))
                d6['rounds'] = int(data.get('n_rounds_k_core_rounds', None))
            ...
            if ('cold_users' in prefiltering_strategies):
                ...
                d7['threshold'] = int(data.get('cold_users_threshold', None))
        ...
        config['experiment']['splitting']['test_splitting']['strategy'] = data.get('test_splitting_strategy',
                                                                                           None)
        if data['test_splitting_strategy']:  # deve attivarsi sempre perché è obbligatorio
            if data.get('test_splitting_strategy') == 'fixed_timestamp':
                # gestione test_fixed_timestamp
                print('Selected a fixed timestamp test splitting strategy ')
                if data.get('test_fixed_timestamp_value') == 'best':
                    config['experiment']['splitting']['test_splitting']['timestamp'] = data.get(
                        'test_fixed_timestamp_value')
                else:
                    config['experiment']['splitting']['test_splitting']['timestamp'] = int(data.get(
                        'test_fixed_timestamp_value',
                        None))
            elif data.get('test_splitting_strategy') == 'temporal_hold_out':
                print('Selected a temporal hold out test splitting strategy ')
                if data.get('test_temporal_hold_out_test_ratio'):
                    config['experiment']['splitting']['test_splitting']['test_ratio'] = float(data.get(
                        'test_temporal_hold_out_test_ratio', None))
                else:
                    config['experiment']['splitting']['test_splitting']['leave_n_out'] = int(data.get(
                        'test_temporal_hold_out_test_leave_n_out', None))
            elif data.get('test_splitting_strategy') == 'random_subsampling':
                print('Selected a random subsampling test splitting strategy ')
                if data.get('test_random_subsampling_test_ratio'):
                    config['experiment']['splitting']['test_splitting']['test_ratio'] = float(data.get(
                        'test_random_subsampling_test_ratio', None))
                else:
                    config['experiment']['splitting']['test_splitting']['leave_n_out'] = int(data.get(
                        'test_random_subsampling_leave_n_out', None))
                if data.get('test_random_subsampling_folds'):
                    config['experiment']['splitting']['test_splitting']['folds'] = int(data.get(
                        'test_random_subsampling_folds', 1))

            elif data.get('test_splitting_strategy') == 'random_cross_validation':
        ...
        if data.get('validation_splitting_strategy'):
            config['experiment']['splitting']['validation_splitting'] = dict()
            config['experiment']['splitting']['validation_splitting']['strategy'] = data.get(
                'validation_splitting_strategy', None)
            if data.get('validation_splitting_strategy') == 'fixed_timestamp':
                # gestione validation_fixed_timestamp
                print('Selected a fixed timestamp validation splitting strategy ')
                if data.get('test_splitting_strategy') == 'best':
                    config['experiment']['splitting']['test_splitting']['timestamp'] = data.get(
                    ...
                    config['experiment']['splitting']['test_splitting']['timestamp'] = int(data.get(
                        ...
                        None))
            elif data.get('validation_splitting_strategy') == 'temporal_hold_out':
                print('Selected a temporal hold out validation splitting strategy ')
                if data.get('validation_temporal_hold_out_test_ratio'):
                    config['experiment']['splitting']['validation_splitting']['test_ratio'] = float(data.get(
                        'validation_temporal_hold_out_test_ratio', None))
                else:
                    config['experiment']['splitting']['validation_splitting']['leave_n_out'] = int(data.get(
                        'validation_temporal_hold_out_test_leave_n_out', None))
            elif data.get('validation_splitting_strategy') == 'random_subsampling':
                print('Selected a random subsampling validation splitting strategy ')
                if data.get('validation_random_subsampling_test_ratio'):
                    config['experiment']['splitting']['validation_splitting']['test_ratio'] = float(data.get(
                        'validation_random_subsampling_test_ratio', None))
                else:
                    config['experiment']['splitting']['validation_splitting']['leave_n_out'] = int(data.get(
                        'validation_random_subsampling_leave_n_out', None))
                config['experiment']['splitting']['validation_splitting']['folds'] = int(data.get(
                    'validation_random_subsampling_folds', 1))
            elif data.get('validation_splitting_strategy') == 'random_cross_validation':
                print('Selected a random cross validation splitting strategy ')
                config['experiment']['splitting']['validation_splitting']['folds'] = int(data.get(
                    'validation_random_cross_validation_folds', 1))

    elif data['loading_strategy'] == 'fixed':
    ...
    elif data['loading_strategy'] == 'hierarchy':
    ...
    if data.get('random_seed'):
        config['experiment']['random_seed'] = int(data.get('random_seed'))
    # gestione binarize
    config['experiment']['binarize'] = bool(data.get('binarize', False))
